...
class MultimodalEncoder(nn.Module):
    def __init__(self, config, layer_number):
        ...
        self.input_projection = nn.Linear(768, 512)

    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        hidden_states = self.input_projection(hidden_states) # Fit input_shape from 768 (image) to 512 (text)
        all_encoder_layers = []
...
class MV_ViTVisoBert(nn.Module):
    def __init__(self, args):
        super(MV_ViTVisoBert, self).__init__()
        self.model = ViTModel.from_pretrained("google/vit-base-patch16-224")

        # Load ViSoBERT with AutoModel and AutoConfig for compatibility
        self.visobert = TextModel(args, model_name="uitnlp/visobert")
        ...
        if args.simple_linear:
            ...
            # self.image_linear = nn.Linear(args.image_size, args.text_size)
            ...
            self.image_linear = nn.Sequential(
                # nn.Linear(args.image_size, args.text_size),
                nn.Linear(args.image_size, args.image_size),
                nn.Dropout(args.dropout_rate),
                nn.GELU()
            )

        self.classifier_fuse = nn.Linear(args.text_size * 2, args.label_number)
        ...
        self.att = nn.Linear(args.text_size * 2, 1, bias=False)
    ...
    def forward(
        ...
        # Obtain text and image features using ViSoBERT and ViT
        ...
        # print(f'inputs_image: {inputs_image.keys()}')
        # print(f'inputs_text: {inputs_text.keys()}')

        vit_outputs = self.model(pixel_values=inputs_image['pixel_values'])
        image_features = vit_outputs.last_hidden_state
        image_feature = vit_outputs.pooler_output
        
        # Process text and image features
        image_feature = self.image_linear(image_feature)
        ocr_features = self.image_linear(ocr_features)
        
        image_features = torch.cat([image_features, ocr_features], dim=1)
        input_embeds = torch.cat((image_features, text_features), dim=1)

        img_attention_mask = torch.ones((inputs_image['pixel_values'].shape[0], image_features.shape[1])).to(image_features.device)
